...
function addFontSizeOption(container) {
  ...
  input.value = parseInt(
    getComputedStyle(document.querySelector(".terminal")).fontSize
  );
  input.min = 10;
  input.max = 25;
  input.style.width = "60px";
  input.addEventListener("input", (event) => {
    document.querySelector(".terminal").style.fontSize =
...
function addFontFamilyOption(container) {
  ...
  const currentFontFamily = getComputedStyle(
...
function addThemeOption(container) {
  ...
  select.value =
    currentBackgroundColor === "rgba(0, 0, 30, 1)" ? "Green" : "Black";
...
export function applyTheme(theme) {
  ...
  if (theme === "Green") {
    ...
    terminal.style.backgroundImage =
      "radial-gradient(ellipse, #05321e 0%, #050505 90%)";
    ...
    terminal.style.backgroundImage =
      "radial-gradient(circle, hsla(0, 0%, 0%, 1) 0%, hsla(0, 0%, 0%, 1) 100%)";
